...
void handleClient(int fd, ServerState &state)
{
    while (true)
    {
        ...
        if (cmd == "LOGOUT")
            ...
            cmdCall(fd, parts, state);
        else if (cmd == "CALL_ACCEPT")
            cmdCallAccept(fd, parts, state);
        else if (cmd == "CALL_PORT")
            cmdCallPort(fd, parts, state);
        else if (cmd == "CALL_REJECT")
            cmdCallReject(fd, parts, state);
        else if (cmd == "CALL_END")
            cmdCallEnd(fd, parts, state);
        else if (cmd == "DH_INIT")
